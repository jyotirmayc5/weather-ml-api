...
**Concrete implication going forward**: any future model training/calibration/backtesting needs to use Kalshi's real settlement values as the label, not our own `actual_high_f` — or at minimum, always evaluate against Kalshi's values even if `actual_high_f` stays as an internal NWS-fidelity metric. Not yet built: an ongoing job to pull and store Kalshi's daily settlement value (the two scripts above are one-off analysis, not scheduled) — worth doing before Phase 2 goes further, so this doesn't need re-pulling by hand each time.
